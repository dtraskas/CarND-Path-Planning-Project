...
HighwayMap::HighwayMap(std::string filename) 
{
    std::ifstream in_map_(filename.c_str(), ifstream::in);
    string line;
...
{
    int closestWaypoint = HighwayMap::ClosestWaypoint(x,y);
...
{
    int next_wp = HighwayMap::NextWaypoint(x, y, theta);
